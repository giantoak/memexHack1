...
sumStat <- function(dataset, variableName, denomName = NULL) {
  ...
  summaryStats$Value[summaryStats$Stats=='Number NA'] <- nrow(dataset[is.na(dataset[[variableName]]), ])
  summaryStats$Value[summaryStats$Stats=='Percent NA'] <- nrow(dataset[is.na(dataset[[variableName]]), ])/nrow(dataset) 
