...
market_segments = ['is_massage_parlor_ad']
time = ['month', 'year']
...
panel_frame = panel_frame.reset_index()
...
print(m.shape)
m=m.set_index(time_place_market_segments)
m=m.sort_index(ascending=True)  # Sort by index, so that groupby shift makes sense
...
annual_normalization = pd.DataFrame(price_index.xs(2013, level='year')
                                    ...
                                    .mean()).reset_index()
annual_normalization = annual_normalization.rename(columns={'price_index': 'normalization'})
